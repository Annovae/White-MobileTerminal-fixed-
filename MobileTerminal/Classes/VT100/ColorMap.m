...
- (id)initWithCoder:(NSCoder *)decoder
{
  ...
  if (self != nil) {    
    ...
    if ([decoder containsValueForKey:@"background"]) {
      ...
      background = [[UIColor colorWithRed:1.0f green:1.0f blue:1.0f alpha:1.0f] retain];
    }
    if ([decoder containsValueForKey:@"foreground"]) {
      foreground = [[decoder decodeObjectForKey:@"foreground"] retain];
    } else {
      foreground = [[UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.95f] retain];
    }
    ...
    if ([decoder containsValueForKey:@"backgroundCursor"]) {
      ...
      backgroundCursor = [[UIColor colorWithRed:0.0f green:0.0f blue:0.0f alpha:0.4f] retain];
    }
  }
  return self;
}
